...
function makeMove(row, col) {
  if (gameBoard[row][col] === "") {
    ...
    // let playerCell = document.getElementById(`cell-${row}-${col}`);
    // console.log("Player Cell:", playerCell);

    // playerCell.textContent = currentPlayer;
    // playerCell.setAttribute("id", `${currentPlayer} cell-${row}-${col}`);

    // if (currentPlayer === "X") {
    //   playerCell.style.backgroundColor = "red";
    //   console.log("This is my player cell:", playerCell);
...
function restartGame() {
  ...
  document.querySelectorAll(".cell").forEach((cell) => (cell.id = ""));
}
